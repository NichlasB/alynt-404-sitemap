...
<div class="alynt-404-settings-section">
	<h2><?php esc_html_e( '404 Page Content', 'alynt-404-sitemap' ); ?></h2>
		...
		<tr>
			...
			<td>
				<div class="alynt-404-media-upload">
					<input type="hidden" 
					name="<?php echo esc_attr( ALYNT_404_PREFIX . '404_settings[featured_image]' ); ?>" 
					id="featured_image" 
	...
	<tr>
		...
		<td>
			<input type="text" 
			id="404_heading" 
			name="<?php echo esc_attr( ALYNT_404_PREFIX . '404_settings[heading]' ); ?>" 
			value="<?php echo esc_attr( $settings['heading'] ?? __( "Oops! That page can't be found.", 'alynt-404-sitemap' ) ); ?>" 
			class="large-text"
			required />
			<p class="description">
				<?php esc_html_e( 'Main heading displayed on the 404 page.', 'alynt-404-sitemap' ); ?>
			</p>
		</td>
	</tr>
	...
	<tr>
		<th scope="row">
			<label for="404_message">
				<?php esc_html_e( 'Error Message', 'alynt-404-sitemap' ); ?>
			</label>
		</th>
		<td>
			<textarea id="404_message" 
			name="<?php echo esc_attr( ALYNT_404_PREFIX . '404_settings[message]' ); ?>" 
			class="large-text" 
			rows="3" 
			required><?php echo esc_textarea( $settings['message'] ?? __( "Looks like this page took a wrong turn. Let's get you back to where you need to be.", 'alynt-404-sitemap' ) ); ?></textarea>
			<p class="description">
				<?php esc_html_e( 'Message displayed below the heading.', 'alynt-404-sitemap' ); ?>
			</p>
		</td>
	</tr>
	...
	<tr>
		<th scope="row">
			<?php esc_html_e( 'Search Content Types', 'alynt-404-sitemap' ); ?>
		</th>
		<td>
			<fieldset>
				<legend class="screen-reader-text">
					<?php esc_html_e( 'Select content types to include in search results', 'alynt-404-sitemap' ); ?>
				</legend>
				<?php foreach ( $post_types as $current_post_type => $object ) : ?>
					<label>
						<input type="checkbox" 
						name="<?php echo esc_attr( ALYNT_404_PREFIX . '404_settings[search_post_types][]' ); ?>" 
						value="<?php echo esc_attr( $current_post_type ); ?>"
						<?php checked( in_array( $current_post_type, $settings['search_post_types'] ?? array( 'post', 'page' ), true ) ); ?> />
						<?php echo esc_html( $object->labels->name ); ?>
					</label><br>
				<?php endforeach; ?>
				<p class="description">
					<?php esc_html_e( 'Select which content types should appear in the AJAX search results.', 'alynt-404-sitemap' ); ?>
				</p>
			</fieldset>
		</td>
	</tr>
...
<div class="alynt-404-settings-section">
	<h2><?php esc_html_e( 'Quick Links', 'alynt-404-sitemap' ); ?></h2>
	<div class="alynt-404-button-links" data-max-buttons="4">
				...
				foreach ( $button_links as $index => $button ) :
					...
					<div class="button-link-item" data-index="<?php echo esc_attr( $index ); ?>">
						<div class="button-link-fields">
							<input type="text" 
									name="<?php echo esc_attr( sprintf( '%s404_settings[button_links][%d][text]', ALYNT_404_PREFIX, (int) $index ) ); ?>" 
									value="<?php echo esc_attr( $button['text'] ); ?>"
									placeholder="<?php esc_attr_e( 'Button Text', 'alynt-404-sitemap' ); ?>" 
									aria-label="
									<?php
									/* translators: %d: 1-based position of quick link button. */
									printf( esc_attr__( 'Quick link %d: button text', 'alynt-404-sitemap' ), absint( $index ) + 1 );
									?>
									"
									class="button-link-text regular-text" />
							<input type="text" 
									name="<?php echo esc_attr( sprintf( '%s404_settings[button_links][%d][url]', ALYNT_404_PREFIX, (int) $index ) ); ?>" 
									value="<?php echo esc_url( $button['url'] ); ?>"
									placeholder="<?php esc_attr_e( 'Button URL', 'alynt-404-sitemap' ); ?>" 
									aria-label="
									<?php
									/* translators: %d: 1-based position of quick link button. */
									printf( esc_attr__( 'Quick link %d: button URL', 'alynt-404-sitemap' ), absint( $index ) + 1 );
									?>
									"
									class="button-link-url regular-text" />
						</div>
						<div class="button-link-actions">
							<button type="button" class="button-link-move-up" aria-label="<?php esc_attr_e( 'Move up', 'alynt-404-sitemap' ); ?>">
								<span class="dashicons dashicons-arrow-up-alt2" aria-hidden="true"></span>
							</button>
							<button type="button" class="button-link-move-down" aria-label="<?php esc_attr_e( 'Move down', 'alynt-404-sitemap' ); ?>">
								<span class="dashicons dashicons-arrow-down-alt2" aria-hidden="true"></span>
							</button>
							<button type="button" class="button-link-remove" aria-label="<?php esc_attr_e( 'Remove', 'alynt-404-sitemap' ); ?>">
		...
		<div class="button-link-add">
			<button type="button" class="button add-button-link">
				<?php esc_html_e( 'Add Quick Link', 'alynt-404-sitemap' ); ?>
			</button>
			<p class="description">
				<?php esc_html_e( 'Add quick navigation links that will appear as buttons on the 404 page. Maximum 4 buttons per row.', 'alynt-404-sitemap' ); ?>
			</p>
		</div>
	</div>
</div>
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td>
				...
				name="<?php echo esc_attr( ALYNT_404_PREFIX . '404_settings[meta_description]' ); ?>" 
				class="large-text" 
				rows="3" 
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td>
				...
				name="<?php echo esc_attr( ALYNT_404_PREFIX . '404_settings[custom_css]' ); ?>" 
				class="large-text code" 
				rows="10"><?php echo esc_textarea( $settings['custom_css'] ?? '' ); ?></textarea>
				<p class="description">
					<?php esc_html_e( 'Add custom CSS styles for the 404 page. These styles will only apply to the 404 page.', 'alynt-404-sitemap' ); ?>
...
<script type="text/template" id="button-link-template">
	<div class="button-link-item" data-index="{{index}}">
		<div class="button-link-fields">
			<input type="text" 
					name="<?php echo esc_attr( ALYNT_404_PREFIX . '404_settings[button_links][{{index}}][text]' ); ?>" 
					placeholder="<?php esc_attr_e( 'Button Text', 'alynt-404-sitemap' ); ?>" 
					aria-label="<?php esc_attr_e( 'Quick link: button text', 'alynt-404-sitemap' ); ?>" 
					class="button-link-text regular-text" />
			<input type="text" 
					name="<?php echo esc_attr( ALYNT_404_PREFIX . '404_settings[button_links][{{index}}][url]' ); ?>" 
					placeholder="<?php esc_attr_e( 'Button URL', 'alynt-404-sitemap' ); ?>" 
					aria-label="<?php esc_attr_e( 'Quick link: button URL', 'alynt-404-sitemap' ); ?>" 
					class="button-link-url regular-text" />
		</div>
		<div class="button-link-actions">
			<button type="button" class="button-link-move-up" aria-label="<?php esc_attr_e( 'Move up', 'alynt-404-sitemap' ); ?>">
